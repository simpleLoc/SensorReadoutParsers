classdef Madgwick < handle
	...
	methods
		function self = Madgwick(sampleFrequency, beta)
			if isOctave()
				pkg load quaternion;
			end
			self.sampleFrequency = double(sampleFrequency);
			self.beta = double(beta);
			self.quat = [1;0;0;0];
		end

		function fastStart(self, accel, gyro)
			...
			% beta weight to 100% accelerometer, and disable the division
			% through the sampleFrequency.
			self.beta = beta * self.sampleFrequency;

			% run adaption
			self.push(accel, gyro);

			% restore to actual settings
			self.beta = beta;
		end

		function q = push(self, accel, gyro)
			q = self.quat;
