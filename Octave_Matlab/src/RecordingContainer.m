classdef RecordingContainer < handle
	...
	methods
		function [recordingTimestamps, recordingDataMatrix] = getSensorData(self, sensorSelector)
			...
			recordingDataMatrix = zeros(sensorSelector.getTotalChannelCnt(), length(recordingTimestamps));
			...
			virtualSensors = sensorSelector.getVirtualSensors();
			for s = 1:length(virtualSensors)
				virtualSensor = virtualSensors{s};
				sensorChannelCnt = virtualSensor.getChannelCnt();
				recordingDataMatrix(matrixRowOffset:matrixRowOffset+sensorChannelCnt-1, :) = virtualSensor.calculate(self);
				matrixRowOffset = matrixRowOffset + sensorChannelCnt;
			end
		end
	end
